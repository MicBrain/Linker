...
#------------------------------------------------------------------------------
inst_needs_relocation:	srl $t3, $a0, 26
			li $t2, 3
			li $t1, 2
			beq $t1, $t3, need_relocation
			beq $t2, $t3, need_relocation
			j no_relocation

need_relocation:	li $v0, 1
			jr $ra

no_relocation:		li $v0, 0	
			jr $ra
...
relocate_inst:	addiu $sp, $sp, -20
		...
		jal addr_for_symbol
		li $t0, -1
		beq $v0, $t0, does_not_exist
		j alteration

alteration:	lw $a0, 4($sp)
		andi $a0, $a0, 0xFC000000
		srl $v0, $v0, 2
		or $v0, $a0, $v0
		j last

does_not_exist:	li $v0, -1
		j last
